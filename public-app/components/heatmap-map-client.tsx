...
// Crime type → marker colour mapping
...
// Nominatim geocode: returns [lat, lng] or null
async function geocode(query: string): Promise<[number, number] | null> {
  try {
    ...
    const res = await fetch(url, {
      headers: { "Accept-Language": "en", "User-Agent": "THUNA-CDMS/1.0" },
    });
...
export default function HeatmapMapClient({ firs }: Props) {
  ...
  const [status, setStatus]     = useState<"idle" | "geocoding" | "done" | "error">("idle");
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      // OSM tile layer — same as the case-level Nominatim maps, no API key needed
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '© <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors',
        maxZoom: 18,
      }).addTo(map);

      // Geocode and plot markers one by one
      const unique = firs.slice(0, 40); // Nominatim rate limit: ~1 req/s, cap at 40
            ...
            L.marker(coords, { icon })
              ...
              .bindPopup(`
                <div style="font-family:Inter,sans-serif;min-width:180px">
                  ...
                  <div style="font-size:11px;color:#43474f;margin-bottom:2px">
                    <span style="font-weight:600;color:${color}">${fir.crime_type || "Unknown"}</span>
                  </div>
                  <div style="font-size:11px;color:#43474f;margin-bottom:2px">📍 ${loc}</div>
                  <div style="font-size:11px;color:#43474f">📅 ${date} &nbsp;·&nbsp; 
                    <span style="text-transform:capitalize;font-weight:600">${fir.status || ""}</span>
                  </div>
                </div>
              `);
          }
          setResolved((r) => r + 1);
          // ~1.1s between requests to respect Nominatim usage policy
    ...
    <div className="relative w-full" style={{ height: "calc(100vh - 200px)", minHeight: 520 }}>
      ...
      {status === "done" && resolved > 0 && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[500] flex items-center gap-2 px-4 py-2 rounded-xl shadow"
          ...
          <span style={{ color: "#a9c7ff", fontSize: 12, fontWeight: 600 }}>
            {resolved} case{resolved !== 1 ? "s" : ""} plotted
          </span>
        </div>
      )}
    </div>
